...
namespace sict {
	...
	Contact::Contact(const Contact& con) {
		init(con.contact_name, con.phone_numbers, con.no_phone_numbers);
	...
	Contact& Contact::operator+=(long long phone_no) {

		if (isValid(phone_no)) {
			no_phone_numbers += 1;
			long long* temp_obj = new long long[no_phone_numbers];

			for (int i = 0; i < no_phone_numbers - 1; i++)
			{
				temp_obj[i] = phone_numbers[i];
			}
			temp_obj[no_phone_numbers - 1] = phone_no;
